...
def normalize(values, mean=0., std=1.):
    values = (values - np.mean(values)) / (np.std(values) + 1e-7)
    return mean + ((std + 1e-7) * values)
...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            x = tf.layers.dense(x, size, activation=activation, name='h{}'.format(i + 1))
        output_placeholder = tf.layers.dense(x, output_size, activation=output_activation, name='output')
...
class Agent(object):
    ...
    def init_tf_sess(self):
        ...
        tf_config.gpu_options.allow_growth = True   # FIX: Necessary to run multiple seeds without all but one crashing
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, 'policy_nn', self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            sy_sampled_ac = tf.random_normal(tf.shape(sy_mean), mean=sy_mean, stddev=tf.exp(sy_logstd))
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            # Log probability under a categorical distribution is equal to the negative cross_entropy
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            #                           ----------PROBLEM 3----------
            #====================================================================================#
            ac = self.sess.run(self.sy_sampled_ac, feed_dict={self.sy_ob_no: ob[np.newaxis]})     # YOUR CODE HERE
            ac = ac[0]
            acs.append(ac)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            def _trajectory_rewards(re):
                q = np.empty_like(re)
                q[-1] = re[-1]  # Set the reward to go of the last timestep to the last reward
                # Iterate backwards to compute the rewards to go for each timestep
                for timestep in range(len(re) - 2, -1, -1):
                    q[timestep] = re[timestep] + self.gamma * q[timestep + 1]
                return q
        else:
            def _trajectory_rewards(re):
                # Only compute the reward for one timestep in the trajectory, and copy value
                # This reduces computation time, since all rewards are the same for a given trajectory
                return [np.sum(np.power(self.gamma, np.arange(len(re))) * re)] * len(re)

        q_n = np.concatenate([_trajectory_rewards(re) for re in re_n])
        return q_n
